...
# supabase client
supa_client = create_client(os.environ["SUPABASE_URL"], os.environ["SUPABASE_SECRET_KEY"])
...
def save_reading_to_sql(occupancy_result: dict,
                        compact_data: dict,
    ...
    try:
        ...
        forecast = _predict_occupancy_heuristic(sid, horizon_hours=24)
        hour_pred = forecast[0]["expected_occupancy"] if forecast else 0.0
        day_pred = forecast[23]["expected_occupancy"] if len(forecast) >= 24 else 0.0

        result = supa_client.rpc("sensor_usage_last_week", {"sid": sid}).execute()
        usage_rate = result.data if result.data is not None else 0.0

        # update sensor row
        supa_client.table("sensors").update({
            "current_occupancy": int(occupancy_result["occupancy"]),
            "current_avg_temp": float(occupancy_result.get("room_temperature", 0.0)),
            "avg_usage_last_week": usage_rate,
            "hour_from_now": hour_pred,
            "day_from_now": day_pred,
            "fever_detected": int(occupancy_result.get("fever_count", 0)) > 0,
            "last_updated": ts,
        }) \
        .eq("sensor_id", sid).execute()

        # insert reading row
        supa_client.table("readings").insert({
            "sensor_id": sid,
            "recorded_at": ts,
            "avg_temp": float(np.mean(compact_data["t"])),
            "occupancy_count": int(occupancy_result["occupancy"]),
            "pixel_data": compact_data.get("t", []),
        }).execute()

    except Exception as e:
        print(f"Error saving occupancy data to supabase SQL ({type(e).__name__}); will retry on next call.")
...
def save_reading_to_blob(occupancy_result: dict) -> None:
    if not SAVE_LOCAL_DATA and not SAVE_TO_BLOB:
        return
    try:
        timestamp = datetime.now(timezone.utc)
...
def receive_thermal_data(data: dict) -> dict:
    ...
    save_reading_to_blob(occupancy_result)
    save_reading_to_sql(occupancy_result, compact_data, timestamp_iso=now_iso)
